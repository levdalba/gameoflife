...
class GameOfLifeUI:
    def __init__(self, screen: Screen):
        ...
        self.timeout = 0.33
        self.actions_handlers = {ord("q"): self.exit, ord("s"): self.pause}
        self.paused_handlers = {ord(" "): self.invert_cell, ord("g"): self.place_glider}
        self.run()
    ...
    def print_board_to_screen(self):
        self.screen.fill_polygon(
            [
                [
                    (0, 0),
                    (self.screen.width // 2, 0),
                    (self.screen.width // 2, self.screen.height),
                    (0, self.screen.height),
                ]
            ]
        )
        for row_num, row in enumerate(str(self.board).split()):
            self.screen.print_at(row, 1, row_num + 1)

    def print_at(self, text, x_pos, y_pos):
        """Output one or few lines at position on the screen."""
    ...
    def print_help(self):
        ...
        help_lines = (
            ...
            "Press <space> for placing a cell",
